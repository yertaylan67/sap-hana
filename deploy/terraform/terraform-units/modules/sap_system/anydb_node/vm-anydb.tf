...
resource "azurerm_linux_virtual_machine" "dbserver" {
  ...
  proximity_placement_group_id = length(local.anydb_vms) == length(local.zones) ? var.ppg[count.index % length(local.zones)].id : local.zonal_deployment ? var.ppg[count.index % length(local.zones)].id : var.ppg[0].id
