...
resource "azurerm_network_interface" "nics-dbnodes-admin" {
  ...
  name                          = format("%s%s",local.hdb_vms[count.index].name,  var.resource_suffixes["admin-nic"])
...
resource "azurerm_network_interface" "nics-dbnodes-db" {
  count                         = local.enable_deployment ? length(local.hdb_vms) : 0
  name                          = format("%s%s",local.hdb_vms[count.index].name,  var.resource_suffixes["db-nic"])
  location                      = var.resource-group[0].location
  resource_group_name           = var.resource-group[0].name
  enable_accelerated_networking = true
...
resource "azurerm_lb" "hdb" {
  ...
  name                = format("%s%s", local.prefix,  var.resource_suffixes["db-alb"])
  resource_group_name = var.resource-group[0].name
  location            = var.resource-group[0].location
...
resource "azurerm_lb_backend_address_pool" "hdb" {
  count               = local.enable_deployment ? 1 : 0
  name                = format("%s%s", local.prefix,  var.resource_suffixes["db-alb-bepool"])
  resource_group_name = var.resource-group[0].name
...
resource "azurerm_lb_probe" "hdb" {
  ...
  resource_group_name = var.resource-group[0].name
  loadbalancer_id     = azurerm_lb.hdb[count.index].id
  name                = format("%s%s", local.prefix,  var.resource_suffixes["db-alb-hp"])
...
resource "azurerm_lb_rule" "hdb" {
  ...
  frontend_ip_configuration_name = format("%s%s", local.prefix,  var.resource_suffixes["db-alb-feip"])
  backend_address_pool_id        = azurerm_lb_backend_address_pool.hdb[0].id
...
resource "azurerm_availability_set" "hdb" {
  count                        = local.enable_deployment ? 1 : 0
  name                         = format("%s%s", local.prefix,  var.resource_suffixes["db-avset"])
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
...
resource "azurerm_linux_virtual_machine" "vm-dbnode" {
  ...
  dynamic "os_disk" {
    ...
    content {
      name                 = format("%s%s",local.hdb_vms[count.index].name, var.resource_suffixes["osdisk"])
