...
resource "azurerm_network_interface" "app" {
  ...
  name                          = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.app_virtualmachine_names[count.index], var.resource_suffixes["nic"]) : format("%s%s", var.app_virtualmachine_names[count.index], var.resource_suffixes["nic"])
...
resource "azurerm_linux_virtual_machine" "app" {
  ...
  name                         = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.app_virtualmachine_names[count.index], var.resource_suffixes["vm"]) : format("%s%s", var.app_virtualmachine_names[count.index], var.resource_suffixes["vm"])
  computer_name                = var.app_virtualmachine_names[count.index]
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
  ...
  os_disk {
    name                 = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.app_virtualmachine_names[count.index], var.resource_suffixes["osdisk"]) : format("%s%s", var.app_virtualmachine_names[count.index], var.resource_suffixes["osdisk"])
    caching              = "ReadWrite"
...
resource "azurerm_managed_disk" "app" {
  ...
  name                 = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.app_virtualmachine_names[count.index], local.app-data-disks[count.index].suffix) : format("%s%s", var.app_virtualmachine_names[count.index], local.app-data-disks[count.index].suffix)
  location             = var.resource-group[0].location
  resource_group_name  = var.resource-group[0].name
  create_option        = "Empty"
