...
resource "azurerm_linux_virtual_machine" "app" {
  ...
  availability_set_id = local.application_server_count == local.app_zone_count ? null : (
    local.app_zone_count > 1 ? (
      azurerm_availability_set.app[count.index % local.app_zone_count].id) : (
      azurerm_availability_set.app[0].id
    )
  )
  proximity_placement_group_id = var.ppg[count.index % local.app_zone_count].id
  zone                         = local.application_server_count == local.app_zone_count ? local.app_zones[count.index % local.app_zone_count] : null
...
resource "azurerm_managed_disk" "app" {
  ...
  zones = local.application_server_count == local.app_zone_count ? (
