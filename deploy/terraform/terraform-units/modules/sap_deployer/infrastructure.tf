...
resource "azurerm_virtual_network" "vnet_mgmt" {
  count               = local.enable_vm ? (local.enable_deployers && ! local.vnet_mgmt_exists) ? 1 : 0 : 0
...
resource "azurerm_subnet" "subnet_mgmt" {
  count                = local.enable_vm ? (local.enable_deployers && ! local.sub_mgmt_exists) ? 1 : 0 : 0
...
resource "azurerm_storage_account" "deployer" {
  count                     = local.enable_vm && local.enable_deployers ? 1 : 0
