...
resource "azurerm_network_interface" "app" {
  ...
  ip_configuration {
    ...
    private_ip_address            = try(local.app_nic_ips [count.index],cidrhost(length(local.sub_app_arm_id) > 0 ? data.azurerm_subnet.subnet-sap-app[0].address_prefixes[0] : azurerm_subnet.subnet-sap-app[0].address_prefixes[0], tonumber(count.index) + local.ip_offsets.app_vm))
...
resource "azurerm_managed_disk" "app" {
  ...
  name                 = format("%s_%s%s", local.prefix,format("%sapp%02d%s%s", lower(local.sid), count.index, upper(local.app_ostype) == "LINUX" ? "l": "w", substr(var.random-id.hex,0,3)),local.app-data-disks[count.index].name)
  location             = var.resource-group[0].location
  resource_group_name  = var.resource-group[0].name
  create_option        = "Empty"
