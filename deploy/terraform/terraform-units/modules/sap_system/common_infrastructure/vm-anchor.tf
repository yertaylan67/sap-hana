...
resource "azurerm_network_interface" "anchor" {
  ...
  ip_configuration {
    ...
    private_ip_address            = local.sub_admin_exists ? try(local.anchor_nic_ips[count.index],cidrhost(local.sub_admin_prefix, (count.index + 5))) : cidrhost(local.sub_admin_prefix, (count.index + 5))
