...
resource "azurerm_subnet" "storage" {
  count                = local.enable_db_deployment &&  local.storage_subnet_needed ? (local.sub_storage_exists ? 0 : 1) : 0
...
data "azurerm_subnet" "storage" {
  count                = local.enable_db_deployment && local.storage_subnet_needed  ? (local.sub_storage_exists ? 1 : 0) : 0
