...
locals {
 ...
 // If the user specifies the secret name of key pair/password in input, the secrets will be imported instead of creating new secrets
  public_key_secret_name  = try(var.public_key_secret_name, "")
  private_key_secret_name = try(var.private_key_secret_name, "")
  password_secret_name    = try(var.password_secret_name, "")
