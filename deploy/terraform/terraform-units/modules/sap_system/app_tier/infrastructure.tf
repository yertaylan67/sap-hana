...
resource "azurerm_lb" "scs" {
  ...
  name                = format("%s%s", local.prefix, var.resource_suffixes["scs-alb"])
  resource_group_name = var.resource-group[0].name
  location            = var.resource-group[0].location

  frontend_ip_configuration {
    name                          = format("%s%s", local.prefix, var.resource_suffixes["scs-alb-feip"])
  ...
  frontend_ip_configuration {
    name                          = format("%s%s", local.prefix, var.resource_suffixes["scs-ers-feip"])
    subnet_id                     = local.sub_app_exists ? data.azurerm_subnet.subnet-sap-app[0].id : azurerm_subnet.subnet-sap-app[0].id
    private_ip_address_allocation = "Static"
...
resource "azurerm_lb_backend_address_pool" "scs" {
  count               = local.enable_deployment ? 1 : 0
  name                = format("%s%s", local.prefix, var.resource_suffixes["scs-alb-bepool"])
  resource_group_name = var.resource-group[0].name
...
resource "azurerm_lb_probe" "scs" {
  ...
  resource_group_name = var.resource-group[0].name
  loadbalancer_id     = azurerm_lb.scs[0].id
  name                = format("%s%s", local.prefix, var.resource_suffixes[count.index == 0 ? "scs-alb-hp" : "scs-ers-hp"])
...
resource "azurerm_lb_rule" "scs" {
  ...
  frontend_ip_configuration_name = format("%s%s", local.prefix, var.resource_suffixes["scs-alb-feip"])
...
resource "azurerm_lb_rule" "ers" {
  ...
  frontend_ip_configuration_name = format("%s%s", local.prefix, var.resource_suffixes["scs-ers-feip"])
  backend_address_pool_id        = azurerm_lb_backend_address_pool.scs[0].id
...
resource "azurerm_availability_set" "scs" {
  count                        = local.enable_deployment ? 1 : 0
  name                         = format("%s%s", local.prefix, var.resource_suffixes["scs-avset"])
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
...
resource "azurerm_availability_set" "app" {
  count                        = local.enable_deployment ? 1 : 0
  name                         = format("%s%s", local.prefix, var.resource_suffixes["app-avset"])
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
  platform_update_domain_count = 20
  platform_fault_domain_count  = 2
...
resource "azurerm_lb" "web" {
  count               = local.enable_deployment ? 1 : 0
  name                = format("%s%s", local.prefix, var.resource_suffixes["web-alb"])
  resource_group_name = var.resource-group[0].name
  location            = var.resource-group[0].location

  frontend_ip_configuration {
    name                          = format("%s%s", local.prefix, var.resource_suffixes["web-alb-feip"])
...
resource "azurerm_lb_backend_address_pool" "web" {
  count               = local.enable_deployment ? 1 : 0
  name                          = format("%s%s", local.prefix, var.resource_suffixes["web-alb-bepool"])
  resource_group_name = var.resource-group[0].name
...
resource "azurerm_availability_set" "web" {
  count                        = local.enable_deployment ? 1 : 0
  name                         = format("%s%s", local.prefix, var.resource_suffixes["web-avset"])
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
  platform_update_domain_count = 20
  platform_fault_domain_count  = 2
