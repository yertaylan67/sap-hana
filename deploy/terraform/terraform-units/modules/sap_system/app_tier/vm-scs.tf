...
resource "azurerm_network_interface" "scs" {
  ...
  name                          = format("%s_%s%02d-nic", local.prefix, local.scs_computername,count.index)
...
resource "azurerm_linux_virtual_machine" "scs" {
  ...
  name                         = format("%s_%s%02d-vm",  local.prefix, local.scs_computername, count.index)
  computer_name                = format("%s%02d",  local.scs_computername, count.index)
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
  ...
  os_disk {
    name                 = format("%s_%s%02d-osdisk", local.prefix,local.scs_computername, count.index)
    caching              = "ReadWrite"
