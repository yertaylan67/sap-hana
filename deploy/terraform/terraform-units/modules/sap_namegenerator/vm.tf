locals {
  ...
  anydb_server_names = flatten([[for idx in range(var.db_server_max_count) :
    lower(format("%sd%s%02dl%d%s", lower(var.sap_sid), lower(local.db_platformcode), idx, 0, substr(var.random-id, 0, 3)))
    ],
    [for idx in range(var.db_server_max_count) :
      lower(format("%sd%s%02dl%d%s", lower(var.sap_sid), lower(local.db_platformcode), idx + var.db_server_max_count, 1, substr(var.random-id, 0, 3)))
    ]
    ]
  )
