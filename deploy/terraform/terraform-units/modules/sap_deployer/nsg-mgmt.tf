...
resource "azurerm_network_security_group" "nsg_mgmt" {
  count               = local.enable_vm ? local.enable_deployers && ! local.sub_mgmt_nsg_exists ? 1 : 0 : 0
...
resource "azurerm_subnet_network_security_group_association" "associate_nsg_mgmt" {
  count                     = local.enable_vm ? local.enable_deployers ? signum((local.vnet_mgmt_exists ? 0 : 1) + (local.sub_mgmt_nsg_exists ? 0 : 1)) : 0 : 0
