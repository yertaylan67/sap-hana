locals {
  ...
  anydb_server_zonal_names = [for idx in range(var.db_server_count) :
    local.zonal_deployment ? format("%sd%s_z%s_%02dl%d%s", lower(var.sap_sid), lower(var.db_sid), var.zones[idx % length(var.zones)], idx, 0, local.random_id_vm_verified) : null
  ]

  anydb_server_zonal_names_ha = [for idx in range(var.db_server_count) :
    local.zonal_deployment ? format("%sd%s_z%s_%02dl%d%s", lower(var.sap_sid), lower(var.db_sid), var.zones[idx % length(var.zones)], idx, 1, local.random_id_vm_verified) : null
  ]
  ...
  hana_server_zonal_names = [for idx in range(var.db_server_count) :
    local.zonal_deployment ? format("%sd%s_z%s_%02dl%d%s", lower(var.sap_sid), lower(var.db_sid), var.zones[idx % length(var.zones)], idx, 0, local.random_id_vm_verified) : null
  ]

  hana_server_zonal_names_ha = [for idx in range(var.db_server_count) :
    local.zonal_deployment ? format("%sd%s_z%s_%02dl%d%s", lower(var.sap_sid), lower(var.db_sid), var.zones[idx % length(var.zones)], idx, 1, local.random_id_vm_verified) : null
  ]
