...
resource "azurerm_network_interface" "scs" {
  ...
  name                          = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.scs_vm_names[count.index], var.resource_suffixes["nic"]) : format("%s%s", var.scs_vm_names[count.index], var.resource_suffixes["nic"])
...
resource "azurerm_linux_virtual_machine" "scs" {
  ...
  name                         = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.scs_vm_names[count.index], var.resource_suffixes["vm"]) : format("%s%s", var.scs_vm_names[count.index], var.resource_suffixes["vm"])
  computer_name                = var.scs_vm_names[count.index]
  location                     = var.resource-group[0].location
  resource_group_name          = var.resource-group[0].name
  ...
  os_disk {
    name                 = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.scs_vm_names[count.index], var.resource_suffixes["osdisk"]) : format("%s%s", var.scs_vm_names[count.index], var.resource_suffixes["osdisk"])
    caching              = "ReadWrite"
...
resource "azurerm_managed_disk" "scs" {
  ...
  name                 = length(local.prefix) > 0 ? format("%s_%s%s", local.prefix, var.scs_vm_names[count.index], local.scs-data-disks[count.index].suffix) : format("%s%s", var.scs_vm_names[count.index], local.scs-data-disks[count.index].suffix)
  location             = var.resource-group[0].location
  resource_group_name  = var.resource-group[0].name
  create_option        = "Empty"
