...
resource "azurerm_linux_virtual_machine" "dbserver" {
  ...
  availability_set_id = local.db_server_count == local.db_zone_count ? null : (
    azurerm_availability_set.anydb[count.index % local.db_zone_count].id
  )
  zone = local.db_server_count == local.db_zone_count ? local.zones[count.index % local.db_zone_count] : null
...
resource "azurerm_managed_disk" "disks" {
  ...
  zones = local.db_server_count == local.db_zone_count ? (
